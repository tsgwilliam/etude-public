...
def aggregate_chart_data(rows: pd.DataFrame, modules: list[str], em_col: str = "kgco2e") -> pd.DataFrame:
    ...
    if rr.empty:
        return pd.DataFrame(columns=["chart_high", "chart_segment_label", "value"])

    return (
        rr.groupby(["chart_high", "chart_segment_label", "chart_segment_code"], dropna=False)[value_col]
        .sum()
        .reset_index()
...
def _build_segments(
    ...
    for high in high_order:
        ...
        seg_order[high] = sub["chart_segment_label"].astype(str).unique().tolist()

    segments: list[dict] = []
    for high in high_order:
        palette = shade_palette(colour_map.get(high, get_default_rics_colour(high)), len(seg_order.get(high, [])))
...
def plot_rics_two_stacks(
    ...
    def stack_segments(modules: list[str]) -> list[dict]:
        rr = filter_rows_for_modules(rows, modules)
        value_col = pick_chart_value_col(rr, em_col)
        agg = (
            rr.groupby(["chart_high", "chart_segment_label", "chart_segment_code"], dropna=False)[value_col]
        ...
        agg["value"] = agg["value"] * scale
        segments, _, _ = _build_segments(agg, colour_map)
        return segments

    upfront_segments = stack_segments(upfront_modules)
    wlc_segments = stack_segments(whole_life_modules)
    ...
    for label, x_pos, total in [
        (f"Upfront<br>{upfront_total:,.0f} {y_unit}", x_upfront, upfront_total),
        (f"Whole life cycle<br>{wlc_total:,.0f} {y_unit}", x_wlc, wlc_total),
    ]:
        fig.add_annotation(x=x_pos, y=-max(upfront_total, wlc_total) * 0.08, text=label, showarrow=False, font=dict(size=14))
