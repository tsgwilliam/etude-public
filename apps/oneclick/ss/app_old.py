...
def load_manual_intensity_rows(
    ...
    df["section"] = df["section"].astype(str).str.strip().replace({"A1-A5": "A1-A3"})

    df["kgco2e"] = pd.to_numeric(df["kgco2e_per_m2_gia"], errors="coerce").fillna(0.0) * pd.to_numeric(df["gia_m2"], errors="coerce").fillna(0.0)
    ...
    out["rics_high_label"] = df["rics_high_label"]
    out["rics_level2_label"] = df["rics_level2_label"]
...
def _read_uploaded_to_tempfile(uploaded_file) -> Path:
    ...
    if name_l.endswith(".csv"):
        ...
        suffix = ".xls"
    tmp = Path(st.session_state.get("_tmp_dir", ".")) / f"_upload{suffix}"
...
def build_stack_by_module_export(
    ...
    rr["stack_id"] = rr["rics_detail"].map(clean_rics_label).astype(str)
    rr["stack_display"] = rr["stack_id"].map(display_map).fillna(rr["stack_id"])
    rr["stack_group"] = rr["stack_id"].map(group_map)
    rr["stack_final"] = rr["stack_group"].where(
        rr["stack_group"].notna() & (rr["stack_group"].astype(str).str.len() > 0),
        rr["stack_display"],
    )
...
def compute_top_contributors_by_stack(
    ...
    rr["lvl2_id"] = rr["rics_detail"].map(clean_rics_label).astype(str)
    rr["lvl2_display"] = rr["lvl2_id"].map(display_map).fillna(rr["lvl2_id"])
    rr["lvl2_group"] = rr["lvl2_id"].map(group_map)
    rr["lvl2_final"] = rr["lvl2_group"].where(
        rr["lvl2_group"].notna() & (rr["lvl2_group"].astype(str).str.len() > 0),
        rr["lvl2_display"],
    )
...
def build_subcat_editor_df(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    rr["_order_key"] = (
        rr["rics_detail"].astype(str).str.extract(r"^(\d+(?:\.\d+)*)", expand=False).fillna("999")
    )

    subcats = (
        rr[["subcategory_id", "_order_key"]]
        .dropna()
...
def sync_subcat_editor_state(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = ["subcategory_id", "display_name", "group_name", "contingency_pct"]
    ...
    existing = st.session_state["subcat_map_df"][cols].copy()

    out = fresh_df[["subcategory_id"]].merge(existing, on="subcategory_id", how="left")
    ...
    out["contingency_pct"] = out["contingency_pct"].fillna(out["subcategory_id"].map(default_cont))
...
def plot_rics_single_stack(
    ...
    def parse_num_tuple(s: str) -> tuple:
        if s is None:
            return (9999,)
        s = str(s).strip()
        m = re.match(r"^(\d+(?:\.\d+)*)", s)
        if not m:
            return (9999,)
        return tuple(int(p) for p in m.group(1).split(".")) or (9999,)
    ...
    agg["high_key"] = agg["rics_high_orig"].map(parse_num_tuple)
    agg["lvl2_key"] = agg["rics_level2_orig"].map(parse_num_tuple)

    highs_present = agg["rics_high_clean"].dropna().unique().tolist()
    high_order = [h for h in RICS_HIGH_ORDER if h in highs_present] + [
...
def render_high_level_legend(legend_df: pd.DataFrame, use_intensity: bool):
    ...
    unit_label = "kgCO₂e/m² GIA" if use_intensity else "kgCO₂e"
    ...
    for chunk in rows_of_items:
        ...
        for col, (_, r) in zip(cols, chunk.iterrows()):
            with col:
                ...
                with textcol:
                    st.markdown(f"**{r['rics_high_clean']}**  \n{r['value']:.0f} {unit_label}")
        ...
        for col, (_, r) in zip(cols, chunk.iterrows()):
            with col:
                ...
                with textcol:
                    st.markdown(
                        f"**{r['rics_high_clean']}**  \n"
                        f"Upfront: {r['upfront_value']:.0f} {unit_label}  \n"
                        f"Life cycle: {r['whole_life_value']:.0f} {unit_label}"
                    )
...
if uploaded_files:
    ...
    building_meta_df = edited_buildings.copy()
    ...
    recon = build_reconciliation_table(rows, em_col)
    recon_summary = summarise_reconciliation(recon, tolerance_kg=recon_tolerance_kg)
    recon_flagged = build_unassigned_rows(rows, em_col, tolerance_kg=recon_tolerance_kg)
    ...
    highs_clean = (
        sorted(set(rows["rics_high_label"].dropna().map(clean_rics_label).unique()))
        if "rics_high_label" in rows.columns else []
    )
    colour_map = {}
    with st.sidebar:
    ...
    with st.sidebar:
        ...
        collapsed_high_level_labels = st.checkbox(
            ...
            value=False,
            help=(
                "Keeps the detailed stack structure and separator lines, "
                "but replaces detailed labels with one label per high-level RICS category."
            ),
        )
       
       
        st.subheader("Subcategory renaming / grouping / contingency")
        st.caption(
            "Edit the label shown on the chart and optionally merge into a group. "
            "If group_name is set, all subcategories with the same group_name become one segment. "
            "Contingency is applied per original subcategory."
        )
        ...
        if "project_contingency_pct" not in st.session_state:
            st.session_state["project_contingency_pct"] = 0.0
        ...
        st.session_state["subcat_map_df"] = edited

    subcat_map_df = st.session_state["subcat_map_df"].copy()
    ...
    for sid, pct in zip(subcat_map_df["subcategory_id"], subcat_map_df["contingency_pct"]):
        try:
            ...
            v = 0.0
        contingency_map[str(sid)] = v / 100.0
    ...
    if chart_choice.startswith("1"):
        coverage_modules = upfront_modules
    elif chart_choice.startswith("2"):
        coverage_modules = wlc_modules_selected
    elif chart_choice.startswith("3"):
        coverage_modules = list(dict.fromkeys(upfront_modules + wlc_modules_selected))
    else:
        coverage_modules = upfront_modules
    ...
    with st.expander("Chart coverage audit", expanded=False):
        ...
        if not audit_df.empty:
            show_cols = [c for c in ["building_name", "section", "element_name", "rics_detail", "source_value", "allocated_value", "gap", "gap_abs"] if c in audit_df.columns]
            st.dataframe(audit_df[show_cols].head(200), width="stretch")

    current_fig = None

    if chart_choice.startswith("1"):
        chart_title = f"Upfront embodied carbon (A1–A5) by RICS category — {building_label}"
        ...
        current_fig = plot_rics_single_stack(
            ...
            target_line_value=target_line_value if show_target_line else None,
            target_line_label=target_line_label,
            target_line_colour=target_line_colour,
            target_line_value_2=target_line_value_2 if show_target_line_2 else None,
        ...
        chart_title = f"Life cycle embodied carbon (excl. B6, B7 and D) by RICS category — {building_label}"
        ...
        chart_title = f"Upfront + life cycle embodied carbon by RICS category — {building_label}"
        ...
        with c2:
            if mass_col:
                fig3b = plot_material_treemap_mass(mass_df, title=f"Mass by material category ({mass_col}) — {building_label}")
                fig3b.update_layout(
                    height=chart_height,
                    plot_bgcolor="rgba(0,0,0,0)",
                    paper_bgcolor="rgba(0,0,0,0)",
    ...
    elem_key = [c for c in ["building_name", "element_name", "rics_detail", "rics_high_code", "rics_high_label"] if c in rows.columns]
    if elem_key and "section" in rows.columns:
        value_col_for_export = pick_chart_value_col(rows, em_col)
        elem = (
            rows.groupby(elem_key + ["section"], dropna=False)[value_col_for_export]
            .sum()
            .reset_index()
            .pivot_table(index=elem_key, columns="section", values=value_col_for_export, aggfunc="sum", fill_value=0.0)
            .reset_index()
        )
