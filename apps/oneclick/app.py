...
def plot_rics_single_stack(
    ...
    for h, seg, v, _, colour in segments:
        ...
        legend_name = h
        showlegend_flag = show_legend and (h not in seen_legend_groups)
...
def build_gla_table1_export(rows: pd.DataFrame) -> pd.DataFrame:
    def _normalise_gla_table1_category(label: str) -> str:
    ...
    if "rics_alloc_label" in rr.columns:
        rr["gla_category"] = rr["rics_alloc_label"].map(_normalise_gla_table1_category)
    if "rics_level2_label" in rr.columns:
        rr["gla_category"] = rr["gla_category"].where(
            rr["gla_category"].astype(str).str.strip() != "",
            rr["rics_level2_label"].map(_normalise_gla_table1_category),
        )
    if "rics_high_label" in rr.columns:
        rr["gla_category"] = rr["gla_category"].where(
            rr["gla_category"].astype(str).str.strip() != "",
            rr["rics_high_label"].map(_normalise_gla_table1_category),
        )

    rr["gla_category"] = rr["gla_category"].fillna("").astype(str).str.strip()

    # keep unmapped rows for debug, but exclude from final table
    ...
    # biogenic / sequestered column
    bio_total_col = pd.Series(0.0, index=mapped.index)
    if "biogenic_kgco2e" in mapped.columns:
        bio_total_col = pd.to_numeric(mapped["biogenic_kgco2e"], errors="coerce").fillna(0.0)
    elif "section" in mapped.columns:
        bio_total_col = pd.Series(0.0, index=mapped.index)
        bioc_mask = mapped["section"].astype(str).str.strip().str.lower().eq("bioc")
        if bioc_mask.any():
            bio_total_col = pd.Series(0.0, index=mapped.index)
            bio_total_col.loc[bioc_mask] = pd.to_numeric(mapped.loc[bioc_mask, value_col], errors="coerce").fillna(0.0)

    mapped["_bio_value"] = bio_total_col

    # module table excludes bioC rows
