...
def _derive_rics_level2_from_alloc(alloc_label: pd.Series, high_label: pd.Series) -> pd.Series:
    ...
    lvl2 = alloc.str.extract(r"^(\d+\.\d+\s+[^:;|]+)", expand=False)
    return lvl2.fillna(high)
...
def _explode_rics_allocations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not alloc_cols:
        out = df.copy()
        out["rics_alloc_label"] = ""
        out["rics_allocated_value"] = 0.0
        return out
...
def _drop_total_like_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Resource" in out.columns:
        resource = _clean_str_series(out["Resource"])
        out = out[resource != ""].copy()
    return out
...
def _build_material_entries(base_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if mass_col and mass_col in working.columns:
        ...
        mass_pref = mass_pref[pd.to_numeric(mass_pref[mass_col], errors="coerce").notna() & (pd.to_numeric(mass_pref[mass_col], errors="coerce") != 0)]
    ...
    stage_cols = [
        c for c in entry_desc.columns
        if c not in set(first_cols + ["_material_entry_id"]) and c not in {"section_norm"}
    ]
    for c in stage_cols:
...
def load_detail_report(path: Path) -> DetailReport:
    ...
    if not entries.empty:
        entry_map = entries[["_material_entry_id"]].copy()
        # recreate the same mapping logic cheaply from base df
        working = _drop_total_like_rows(df).copy()
        mass_col2 = "mass_value" if "mass_value" in working.columns else _find_mass_column(working)
        descriptor_cols = [c for c in [
            "Resource",
            "User input",
            "User input unit",
        ...
        material_id_df["_material_entry_id"] = [f"ME{i+1:06d}" for i in range(len(material_id_df))]
        working = working.merge(material_id_df, on=key_cols, how="left")
        df = df.merge(working[["_source_row_id", "_material_entry_id"]], on="_source_row_id", how="left")
    ...
    # ensure convenience aliases survive explode
...
def material_breakdown(rows: pd.DataFrame, module: str):
    ...
    # collapse allocation explosion back to source rows first
